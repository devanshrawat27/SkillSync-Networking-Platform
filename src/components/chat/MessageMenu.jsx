...
export default function MessageMenu() {
  ...
  const nameFor = (id) => profiles[id]?.name || profiles[id]?.email || id;

  return (
    <Popover>
      <PopoverTrigger asChild>
        <button className="relative p-2 rounded-md hover:bg-accent text-foreground/80" aria-label="Messages">
                ...
                return (
                  ...
                  >
                    <Avatar className="w-8 h-8">
                      ...
                      <AvatarFallback>{(nameFor(c.otherUserId).slice(0, 1)).toUpperCase()}</AvatarFallback>
                    </Avatar>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between gap-2">
                        <span className="font-medium text-sm truncate">{nameFor(c.otherUserId)}</span>
                        {c.lastMessage && (
